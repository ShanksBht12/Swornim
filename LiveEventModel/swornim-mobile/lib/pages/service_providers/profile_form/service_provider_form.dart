...
class ServiceProviderFormData {
  ...
  bool offersVideoServices = false;
...
class _ServiceProviderFormState extends ConsumerState<ServiceProviderForm> {
  ...
  void _initializeControllers() {
    _controllers = {
      ...
      'eventRate': TextEditingController(),
  ...
  Widget _buildPhotographerServices() {
    ...
    return Column(
      children: [
        ...
        SectionCard(
          title: 'Equipment',
          subtitle: 'What equipment do you use?',
          child: CustomTextFormField(
            label: 'Camera Equipment',
            hint: 'e.g., Canon EOS R5, Sony A7 III (comma separated)',
        ...
        SectionCard(
          ...
          child: CustomSwitch(
            ...
            subtitle: 'I also offer videography services',
            value: _formData.offersVideoServices,
            onChanged: (value) {
              setState(() {
                _formData.offersVideoServices = value;
              });
            },
          ),
        ),
  ...
  Widget _buildVenueServices() {
    final venueTypeOptions = [
      'Wedding Hall', 'Conference Room', 'Party Hall', 'Exhibition Space',
      'Banquet Hall', 'Garden Venue', 'Rooftop Venue', 'Restaurant', 'Hotel'
    ];
    ...
    return SectionCard(
      ...
      child: Column(
        children: [
          ...
          PriceInputField(
            label: 'Event Rate',
            hint: 'Rate for complete event coverage',
            controller: _controllers['eventRate']!,
            validator: (value) {
              if (value == null || value.trim().isEmpty) {
                return 'Event rate is required';
              }
              final price = double.tryParse(value);
              if (price == null || price <= 0) {
                return 'Please enter a valid price';
              ...
              if (price < 5000) {
                return 'Minimum event rate should be Rs. 5,000';
              }
              return null;
            },
          ),
  ...
  Map<String, dynamic> _buildSubmissionData() {
    final baseData = {
      ...
      'experienceYears': int.tryParse(_controllers['experienceYears']!.text) ?? 0,
      'location': {
    ...
    switch (widget.providerType) {
      ...
      case ServiceProviderType.photographer:
        baseData.addAll({
          ...
          'eventRate': double.tryParse(_controllers['eventRate']!.text) ?? 0,
          'specializations': _formData.photographySpecializations,
          'equipment': _controllers['equipment']!.text.split(',').map((s) => s.trim()).where((s) => s.isNotEmpty).toList(),
          'offersVideoServices': _formData.offersVideoServices,
        });
        break;
      case ServiceProviderType.venue:
        baseData.addAll({
