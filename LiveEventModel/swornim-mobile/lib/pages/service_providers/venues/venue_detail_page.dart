...
class _VenueDetailPageState extends ConsumerState<VenueDetailPage> with TickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: venueAsync.when(
        data: (venue) {
          final packagesAsync = ref.watch(packagesProvider(venue.userId));
          return SafeArea(
            child: CustomScrollView(
              slivers: [
                _buildHeroSection(theme, venue),
                _buildQuickInfoSection(theme, venue),
                if (venue.location != null) _buildEnhancedMapSection(theme, venue),
        ...
        background: Stack(
          ...
          children: [
            ...
            PageView.builder(
              ...
              itemCount: _getImageCount(venue),
              onPageChanged: (index) {
                setState(() {
              ...
              itemBuilder: (context, index) {
                final imageUrl = _getImageUrl(venue, index);
                if (imageUrl == null) {
                  return Container(
                    decoration: BoxDecoration(
            ...
            if (_getImageCount(venue) > 1)
              Positioned(
                ...
                child: Row(
                  ...
                  children: List.generate(_getImageCount(venue), (index) {
          ...
          child: Column(
            ...
            children: [
              ...
              if (venue.images.isNotEmpty) ...[
                _buildPortfolioSection(theme, venue),
                const SizedBox(height: 20),
              ],
              // Contact info
  ...
  Widget _buildReviewsContent(ThemeData theme, Venue venue) {
    ...
    return ReviewsTab(
      ...
      serviceProviderName: venue.businessName,
  ...
  Future<void> _handleBookingSubmission(WidgetRef ref, ThemeData theme) async {
    try {
      ...
      final packagesAsync = ref.read(packagesProvider(venue?.userId ?? ''));
  ...
  Widget _buildEnhancedMapSection(ThemeData theme, Venue venue) {
    return SliverToBoxAdapter(
      child: AnimatedBuilder(
        animation: _mainAnimationController,
        builder: (context, child) {
                ...
                child: _EnhancedMapCard(
                  ...
                  onTap: () {
                    Navigator.of(context).push(
                      MaterialPageRoute(
                        builder: (_) => SingleProviderMapScreen(provider: venue),
                      ),
                    );
                  },
                ),
  ...
  int _getImageCount(Venue venue) {
    int count = 0;
    if (venue.image.isNotEmpty) count++;
    if (venue.images.isNotEmpty) count += venue.images.length;
    return count > 0 ? count : 1; // Return at least 1 for placeholder
  }

  String? _getImageUrl(Venue venue, int index) {
    if (index == 0 && venue.image.isNotEmpty) {
      return venue.image;
    }
    final portfolioIndex = venue.image.isNotEmpty ? index - 1 : index;
    if (portfolioIndex >= 0 && portfolioIndex < venue.images.length) {
      return venue.images[portfolioIndex];
    }
    return null;
  }

  Widget _buildPortfolioSection(ThemeData theme, Venue venue) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Row(
...
class _EnhancedMapCardState extends State<_EnhancedMapCard> with TickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    final theme = Theme.of(context);
    final location = widget.venue.location!;
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          ...
                          Text(
                            location.name.isNotEmpty ? location.name : 'Venue Location',
                            style: theme.textTheme.titleMedium?.copyWith(
                              color: Colors.white,
                              fontWeight: FontWeight.w600,
                            ),
