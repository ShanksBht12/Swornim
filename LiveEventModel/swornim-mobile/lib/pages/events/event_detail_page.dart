...
class _EventDetailPageState extends ConsumerState<EventDetailPage>
  ...
  Widget _buildTicketQuantitySelector() {
    return Row(
      mainAxisAlignment: MainAxisAlignment.center,
      children: [
        Text('Tickets:', style: const TextStyle(fontWeight: FontWeight.w600)),
        const SizedBox(width: 8),
        IconButton(
          icon: const Icon(Icons.remove),
          onPressed: _ticketQuantity > 1 ? () => setState(() => _ticketQuantity--) : null,
        ),
        Text('$_ticketQuantity', style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
        IconButton(
          icon: const Icon(Icons.add),
          onPressed: _ticketQuantity < _maxTickets ? () => setState(() => _ticketQuantity++) : null,
        ),
        const SizedBox(width: 16),
        Text('Total: NPR ${_totalPrice.toStringAsFixed(2)}', style: const TextStyle(fontWeight: FontWeight.w600)),
      ],
    );
  }
