...
exports.checkInAttendee = async (req, res, next) => {
  try {
    ...
    // Debug log
    console.log('DEBUG CHECK-IN:', { bookingId: booking.id, status: booking.status });
    if (booking.status === 'attended') {
      return res.status(400).json({ data: booking, message: 'Already checked in' });
    }
    if (booking.status !== 'pending' && booking.status !== 'confirmed') {
      return res.status(400).json({ data: booking, message: 'Cannot check in with status: ' + booking.status });
    }
