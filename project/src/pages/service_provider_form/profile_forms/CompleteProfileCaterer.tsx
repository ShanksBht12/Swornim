...
import { ChefHat, MapPin, CheckCircle, AlertCircle, DollarSign, Star, Utensils, User, ImageIcon } from "lucide-react"
// @ts-ignore
  ...
  useEffect(() => {
    if (profile) {
      ...
      setForm({
        ...
        cuisineTypes: (p.cuisineTypes || []).join(", "),
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      form.cuisineTypes
        ...
        .filter(Boolean)
        .forEach((val) => {
          formData.append("cuisineTypes[]", val)
        })
      ...
      form.serviceTypes
        ...
        .filter(Boolean)
        .forEach((val) => {
          formData.append("serviceTypes[]", val)
        })

      if (form.pricePerPerson) formData.append("pricePerPerson", form.pricePerPerson)
      if (form.minGuests) formData.append("minGuests", form.minGuests)
      if (form.maxGuests) formData.append("maxGuests", form.maxGuests)
      ...
      await catererService.createProfile(formData)

      // Upload menu images (if any)
      // if (menuImageFiles.length > 0) {
      //   for (const file of menuImageFiles) {
      //     await catererService.addPortfolioImage(file);
      //   }
      // }
            ...
            <form onSubmit={handleSubmit} className="space-y-8">
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-6">
                  ...
                  {/*
                  <div>
                    <label className="form-label">Menu & Food Images</label>
                    <FileUpload
                      onFilesSelected={(files) => setMenuImageFiles(files)}
                      maxSize={5}
                      multiple={true}
                      label="Upload Menu Images"
                      placeholder="Click to select or drag and drop your food and menu images"
                      disabled={loading}
                    />
                      ...
                      <div className="mt-3 p-3 bg-green-50 rounded-lg border border-green-200">
                        <div className="flex items-center space-x-2">
                          ...
                          <span className="text-sm text-green-800">
                            Selected: {menuImageFiles.length} image{menuImageFiles.length > 1 ? "s" : ""}
                          </span>
                        </div>
                      </div>
                    )}
