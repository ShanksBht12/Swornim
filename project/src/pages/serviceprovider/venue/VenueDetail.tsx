...
import { Star, Award, Eye, MapPin, MessageSquare, Share2, Calendar, CheckCircle, Camera, ArrowLeft, Heart, Phone, ChevronLeft, ChevronRight, X, Users, Wifi, Car, Utensils } from 'lucide-react'
  ...
  return (
    <section className="w-full mb-16">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h2 className="headline-large mb-2">Venue Gallery</h2>
          <p className="body-medium text-slate-500">Explore our beautiful venue spaces</p>
        </div>
      ...
      {images.length > 0 ? (
          ...
          <div className="mb-6">
            ...
            >
              ...
              <div className="absolute bottom-4 left-4 bg-black/50 backdrop-blur-sm text-white px-3 py-1 rounded-full">
                <span className="body-small font-medium">Featured Space</span>
              </div>
            </div>
          </div>
        ...
        <div className="flex items-center justify-center h-96 card bg-gradient-to-br from-slate-50 to-slate-100">
          <div className="text-center space-y-4">
            ...
            <div>
              <h3 className="headline-medium text-slate-600 mb-2">No Venue Images</h3>
              <p className="body-medium text-slate-400">Venue images will be displayed here once uploaded</p>
            </div>
          </div>
        </div>
      )}
...
const QuickInfoSection = ({ venue }: { venue: any }) => {
  const infoCards = [
    {
      icon: Award,
      label: "Status",
      value: venue.user?.userType === "venue" ? "Verified Venue" : "Unverified",
      bgColor: venue.user?.userType === "venue" ? "bg-green-50" : "bg-slate-50",
      iconColor: venue.user?.userType === "venue" ? "text-green-600" : "text-slate-500",
      valueColor: venue.user?.userType === "venue" ? "text-green-700 font-semibold" : "text-slate-600",
    },
    {
      icon: Star,
      label: "Rating",
      value: `${venue.rating || 0} (${venue.totalReviews || 0} reviews)`,
    ...
    },
    {
      icon: MapPin,
      label: "Location",
      value: venue.location?.name || "Location not specified",
      bgColor: "bg-blue-50",
    ...
    },
    {
      icon: Users,
      label: "Capacity",
      value: venue.capacity ? `Up to ${venue.capacity} guests` : "Contact for capacity",
    ...
    },
    {
      icon: CheckCircle,
      label: "Availability",
      value: venue.isAvailable ? "Available Now" : "Currently Booked",
      bgColor: venue.isAvailable ? "bg-green-50" : "bg-red-50",
      iconColor: venue.isAvailable ? "text-green-600" : "text-red-600",
      valueColor: venue.isAvailable ? "text-green-700 font-medium" : "text-red-700 font-medium",
    },
    {
      icon: Calendar,
      label: "Starting Rate",
      value: venue.hourlyRate ? `₹${venue.hourlyRate}/day` : "Contact for pricing",
  ...
  return (
    <div className="mb-12">
      <div className="mb-6">
        ...
        <p className="body-medium text-slate-500">Essential venue details at a glance</p>
      </div>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {card.label === "Availability" && (
                      <div className="flex items-center gap-2 mt-2">
                        <div
                          className={`w-2 h-2 rounded-full ${venue.isAvailable ? "bg-green-500" : "bg-red-500"} animate-pulse`}
                        ></div>
                        <span className="body-small text-slate-500">
                          {venue.isAvailable ? "Ready to book" : "Check back later"}
                        </span>
                      </div>
                    )}
                  </div>
...
const AboutSection = ({ venue }: { venue: any }) => (
  <div className="space-y-10">
    ...
    <div className="card p-8 bg-gradient-to-br from-slate-50 to-white">
      ...
      <div className="prose prose-slate max-w-none">
        <p className="body-large leading-relaxed text-slate-700">
          {venue.description ||
            "Our venue offers a perfect setting for your special events, combining elegant spaces with modern amenities. We are committed to providing exceptional service and creating memorable experiences for you and your guests."}
        </p>
      </div>
    </div>
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
      <div className="card p-6">
        <h4 className="headline-small mb-4 text-slate-800">Event Types</h4>
        <div className="flex flex-wrap gap-3">
          {(venue.eventTypes || []).length > 0 ? (
            venue.eventTypes.map((tag: string, idx: number) => (
            ...
            <div className="text-center py-8 w-full">
              <p className="body-medium text-slate-400">No event types listed yet</p>
              <p className="body-small text-slate-300 mt-1">Event types will be displayed here</p>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="card p-6">
        <h4 className="headline-small mb-4 text-slate-800">Amenities & Features</h4>
        <div className="flex flex-wrap gap-3">
          {(venue.amenities || []).length > 0 ? (
            venue.amenities.map((item: string, idx: number) => (
              <span
                key={idx}
                className="chip hover:scale-105 transition-transform duration-200 flex items-center gap-2"
              >
                {item.toLowerCase().includes("wifi") && <Wifi className="w-3 h-3" />}
                {item.toLowerCase().includes("parking") && <Car className="w-3 h-3" />}
                {item.toLowerCase().includes("catering") && <Utensils className="w-3 h-3" />}
                {item}
              </span>
            ))
          ) : (
            <div className="text-center py-8 w-full">
              <p className="body-medium text-slate-400">No amenities listed yet</p>
              <p className="body-small text-slate-300 mt-1">Amenity details will be displayed here</p>
            </div>
          )}
        </div>
      </div>
    ...
    <div className="card p-8 bg-gradient-to-r from-blue-50 via-white to-purple-50">
      <h4 className="headline-medium mb-8 text-center text-slate-800">Venue Statistics</h4>
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-8">
        <div className="text-center group">
          ...
          <div className="display-medium text-slate-800 font-bold mb-1">{venue.experienceYears || 0}</div>
          <div className="body-small text-slate-500 font-medium">Years Operating</div>
        </div>
        <div className="text-center group">
          <div className="mb-4">
            <div className="inline-flex items-center justify-center w-16 h-16 bg-white rounded-2xl shadow-md group-hover:shadow-lg transition-all duration-300 group-hover:scale-110">
          ...
          <div className="display-medium text-slate-800 font-bold mb-1">{venue.totalReviews || 0}</div>
        ...
        <div className="text-center group">
          ...
          <div className="display-medium text-slate-800 font-bold mb-1">{venue.rating || 0}</div>
        ...
        <div className="text-center group">
          ...
          <div className="body-small text-slate-500 font-medium">Events Hosted</div>
        </div>
      </div>
    </div>
  </div>
      ...
      <div className="text-center py-16">
        <div className="card p-12 bg-gradient-to-br from-slate-50 to-slate-100 max-w-md mx-auto">
          ...
          <p className="body-medium text-slate-500 mb-6">
            We create personalized venue packages tailored to your event requirements and budget.
          </p>
  ...
  return (
    <div className="space-y-8">
      <div className="text-center mb-10">
        <h3 className="headline-large mb-3">Venue Packages</h3>
        <p className="body-large text-slate-600">Choose the perfect package for your special event</p>
      </div>
              ...
              <div className="text-center mb-8">
                ...
                <div className="mb-4">
                  ...
                  <div className="flex items-center justify-center gap-2 mt-2">
                    ...
                    <span className="body-medium text-slate-600">{pkg.durationHours} hours rental</span>
                  </div>
                </div>
              </div>
...
// Add booking/payment state and logic (copied from PhotographerDetail)
const VenueDetail = () => {
  const { id } = useParams()
  ...
  // Fetch latest booking for this venue after booking success
  ...
  const handleCreateBooking = async () => {
    ...
    try {
      ...
      const bookingData: any = {
        serviceProviderId: venue?.user?.id,
        packageId,
        eventDate: date,
        eventTime: time,
        eventLocation,
      ...
      <div className="container py-16">
        <div className="flex items-center justify-center min-h-[400px]">
          <div className="text-center space-y-4">
            ...
            <p className="body-large text-slate-600">Loading venue details...</p>
          </div>
        </div>
      </div>
    )
  ...
  if (!venue) {
    return (
      <div className="container py-16">
        <div className="text-center">
          <div className="card p-12 max-w-md mx-auto">
            <p className="headline-medium text-slate-600 mb-4">Venue Not Found</p>
            <p className="body-medium text-slate-500 mb-6">
      ...
      <div className="container py-8 section-spacing">
        ...
        <div className="grid lg:grid-cols-5 gap-8 mb-16">
          ...
          <div className="lg:col-span-2">
            <div className="sticky top-24">
              <div className="relative aspect-square w-full min-h-[400px]">
                <img
                  src={venue.image || venue.profileImage || venue.user?.profileImage || "/placeholder.svg"}
                  alt={venue.businessName || "Venue"}
              ...
              <div className="mt-6 space-y-4">
                <div>
                  <div className="flex items-center gap-3 mb-2">
                    <h1 className="headline-large text-slate-900">{venue.businessName}</h1>
                    {venue.user?.userType === "venue" && (
          ...
          <div className="lg:col-span-3">
            <QuickInfoSection venue={venue} />
          </div>
        </div>

        {/* Gallery */}
        <GallerySection images={venue.images || venue.portfolioImages || []} name={venue.businessName || ""} />
