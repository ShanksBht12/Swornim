    ...
    <section className="w-full mb-16">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="body-medium text-slate-500">Showcasing our finest photographic work</p>
        </div>
...
const QuickInfoSection = ({ photographer }: { photographer: any }) => {
  const infoCards = [
    {
      icon: Award,
      label: "Status",
      value: photographer.user?.userType === "photographer" ? "Verified Professional" : "Unverified",
      bgColor: photographer.user?.userType === "photographer" ? "bg-green-50" : "bg-slate-50",
      iconColor: photographer.user?.userType === "photographer" ? "text-green-600" : "text-slate-500",
      valueColor: photographer.user?.userType === "photographer" ? "text-green-700 font-semibold" : "text-slate-600",
    },
    {
      icon: Star,
      label: "Rating",
      value: `${photographer.rating || 0} (${photographer.totalReviews || 0} reviews)`,
    ...
    },
    {
      icon: MapPin,
      label: "Location",
      value: photographer.location?.name || "Location not specified",
      bgColor: "bg-blue-50",
    ...
    },
    {
      icon: CheckCircle,
      label: "Availability",
      value: photographer.isAvailable ? "Available Now" : "Currently Busy",
      bgColor: photographer.isAvailable ? "bg-green-50" : "bg-red-50",
      iconColor: photographer.isAvailable ? "text-green-600" : "text-red-600",
      valueColor: photographer.isAvailable ? "text-green-700 font-medium" : "text-red-700 font-medium",
    },
    {
      icon: Eye,
      label: "Profile Views",
    ...
    },
    {
      icon: Calendar,
      label: "Starting Rate",
      value: photographer.hourlyRate ? `₹${photographer.hourlyRate}/hr` : "Contact for pricing",
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {card.label === "Availability" && (
                      <div className="flex items-center gap-2 mt-2">
                        <div
                          className={`w-2 h-2 rounded-full ${photographer.isAvailable ? "bg-green-500" : "bg-red-500"} animate-pulse`}
                        ></div>
                        <span className="body-small text-slate-500">
                          {photographer.isAvailable ? "Ready to book" : "Check back later"}
                        </span>
                      </div>
                    )}
                  </div>
...
const AboutSection = ({ photographer }: { photographer: any }) => (
  <div className="space-y-10">
      ...
      <div className="prose prose-slate max-w-none">
        <p className="body-large leading-relaxed text-slate-700">
          {photographer.description ||
            "We are passionate photography professionals dedicated to capturing your most precious moments with artistic vision and technical excellence. Our commitment to storytelling through imagery sets us apart in the industry."}
        </p>
      </div>
    </div>
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
      <div className="card p-6">
        ...
        <div className="flex flex-wrap gap-3">
          {(photographer.specializations || []).length > 0 ? (
            photographer.specializations.map((tag: string, idx: number) => (
      ...
      <div className="card p-6">
        <h4 className="headline-small mb-4 text-slate-800">Equipment & Gear</h4>
        <div className="flex flex-wrap gap-3">
          {(photographer.equipment || []).length > 0 ? (
            photographer.equipment.map((item: string, idx: number) => (
    ...
    <div className="card p-8 bg-gradient-to-r from-blue-50 via-white to-purple-50">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-8">
        <div className="text-center group">
          ...
          <div className="display-medium text-slate-800 font-bold mb-1">{photographer.experienceYears || 0}</div>
        ...
        <div className="text-center group">
          ...
          <div className="display-medium text-slate-800 font-bold mb-1">{photographer.totalReviews || 0}</div>
        ...
        <div className="text-center group">
          ...
          <div className="display-medium text-slate-800 font-bold mb-1">{photographer.rating || 0}</div>
        ...
        <div className="text-center group">
          ...
          <div className="body-small text-slate-500 font-medium">Photo Sessions</div>
        </div>
      </div>
    </div>
  </div>
      ...
      <div className="text-center py-16">
        <div className="card p-12 bg-gradient-to-br from-slate-50 to-slate-100 max-w-md mx-auto">
          ...
          <p className="body-medium text-slate-500 mb-6">
            We create personalized photography packages tailored to your event needs and budget.
          </p>
  ...
  return (
    <div className="space-y-8">
      <div className="text-center mb-10">
        <h3 className="headline-large mb-3">Photography Packages</h3>
        <p className="body-large text-slate-600">Choose the perfect package for your special moments</p>
      </div>
              ...
              <div className="text-center mb-8">
                ...
                <div className="mb-4">
                  ...
                  <div className="flex items-center justify-center gap-2 mt-2">
                    ...
                    <span className="body-medium text-slate-600">{pkg.durationHours} hours coverage</span>
                  </div>
                </div>
              </div>
...
const PhotographerDetail = () => {
  ...
  // Booking logic state (not rendered)
  ...
  // Fetch latest booking for this photographer after booking success
  ...
  // Booking logic function (not rendered)
  const handleCreateBooking = async () => {
    ...
    try {
      ...
      const bookingData: any = {
        serviceProviderId: photographer?.user?.id,
        packageId,
        eventDate: date,
        eventTime: time,
        eventLocation,
  ...
  // Fetch latest booking for this photographer after booking success
  useEffect(() => {
    if (bookingSuccess && photographer?.user?.id) {
      // Fetch all bookings and get the latest for this photographer
      bookingService.getBookings().then((data: any) => {
        // Find the latest booking for this photographer
        const bookings = Array.isArray(data?.data) ? data.data : [];
        const myBooking = bookings
          .filter((b: any) => b.serviceProviderId === photographer.user.id && b.packageId === selectedPackage?.id)
      ...
      <div className="container py-16">
        <div className="flex items-center justify-center min-h-[400px]">
          <div className="text-center space-y-4">
            ...
            <p className="body-large text-slate-600">Loading photographer details...</p>
          </div>
        </div>
      </div>
    )
  ...
  if (!photographer) {
    return (
      <div className="container py-16">
        <div className="text-center">
          <div className="card p-12 max-w-md mx-auto">
            <p className="headline-medium text-slate-600 mb-4">Photographer Not Found</p>
            <p className="body-medium text-slate-500 mb-6">
      ...
      <div className="container py-8 section-spacing">
        ...
        <div className="grid lg:grid-cols-5 gap-8 mb-16">
          ...
          <div className="lg:col-span-2">
            <div className="sticky top-24">
              <div className="relative aspect-square w-full min-h-[400px]">
                <img
                  src={photographer.profileImage || "/placeholder.svg?height=400&width=400"}
              ...
              <div className="mt-6 space-y-4">
                <div>
                  <div className="flex items-center gap-3 mb-2">
                    <h1 className="headline-large text-slate-900">{photographer.businessName}</h1>
                    {photographer.user?.userType === "photographer" && (
          ...
          <div className="lg:col-span-3">
            <QuickInfoSection photographer={photographer} />
          </div>
        </div>

        {/* Gallery */}
        <GallerySection images={photographer.portfolioImages || []} name={photographer.businessName || ""} />

        {/* Main Content */}
        <div className="grid lg:grid-cols-3 gap-12">
          {/* Left Column - About & Packages */}
          <div className="lg:col-span-2 space-y-16">
            <AboutSection photographer={photographer} />
