    ...
    <section className="w-full mb-16">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="body-medium text-slate-500">Showcasing our finest makeup artistry</p>
        </div>
...
const QuickInfoSection = ({ makeupArtist }: { makeupArtist: any }) => {
  const infoCards = [
    {
      icon: Award,
      label: "Status",
      value: makeupArtist.user?.userType === "makeupArtist" ? "Verified Professional" : "Unverified",
      bgColor: makeupArtist.user?.userType === "makeupArtist" ? "bg-green-50" : "bg-slate-50",
      iconColor: makeupArtist.user?.userType === "makeupArtist" ? "text-green-600" : "text-slate-500",
      valueColor: makeupArtist.user?.userType === "makeupArtist" ? "text-green-700 font-semibold" : "text-slate-600",
    },
    {
      icon: Star,
      label: "Rating",
      value: `${makeupArtist.rating || 0} (${makeupArtist.totalReviews || 0} reviews)`,
    ...
    },
    {
      icon: MapPin,
      label: "Location",
      value: makeupArtist.location?.name || "Location not specified",
      bgColor: "bg-blue-50",
    ...
    },
    {
      icon: CheckCircle,
      label: "Availability",
      value: makeupArtist.isAvailable ? "Available Now" : "Currently Busy",
      bgColor: makeupArtist.isAvailable ? "bg-green-50" : "bg-red-50",
      iconColor: makeupArtist.isAvailable ? "text-green-600" : "text-red-600",
      valueColor: makeupArtist.isAvailable ? "text-green-700 font-medium" : "text-red-700 font-medium",
    },
    {
      icon: Eye,
      label: "Profile Views",
    ...
    },
    {
      icon: Calendar,
      label: "Starting Rate",
      value: makeupArtist.hourlyRate ? `₹${makeupArtist.hourlyRate}/hr` : "Contact for pricing",
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {card.label === "Availability" && (
                      <div className="flex items-center gap-2 mt-2">
                        <div
                          className={`w-2 h-2 rounded-full ${makeupArtist.isAvailable ? "bg-green-500" : "bg-red-500"} animate-pulse`}
                        ></div>
                        <span className="body-small text-slate-500">
                          {makeupArtist.isAvailable ? "Ready to book" : "Check back later"}
                        </span>
                      </div>
                    )}
                  </div>
...
const AboutSection = ({ makeupArtist }: { makeupArtist: any }) => (
  <div className="space-y-10">
    ...
    <div className="card p-8 bg-gradient-to-br from-slate-50 to-white">
      <h3 className="headline-medium mb-6 flex items-center gap-3">
        ...
        About Our Makeup Artistry
      </h3>
      <div className="prose prose-slate max-w-none">
        <p className="body-large leading-relaxed text-slate-700">
          {makeupArtist.description ||
            "We are passionate makeup artists dedicated to enhancing natural beauty and creating stunning looks for every occasion. Our expertise in color theory, skin care, and artistic techniques ensures you look and feel your absolute best."}
        </p>
      </div>
    </div>
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
      <div className="card p-6">
        ...
        <div className="flex flex-wrap gap-3">
          {(makeupArtist.specializations || []).length > 0 ? (
            makeupArtist.specializations.map((tag: string, idx: number) => (
      ...
      <div className="card p-6">
        <h4 className="headline-small mb-4 text-slate-800">Products & Tools</h4>
        <div className="flex flex-wrap gap-3">
          {(makeupArtist.equipment || []).length > 0 ? (
            makeupArtist.equipment.map((item: string, idx: number) => (
            ...
            <div className="text-center py-8 w-full">
              <p className="body-medium text-slate-400">No products listed yet</p>
              <p className="body-small text-slate-300 mt-1">Product details will be displayed here</p>
            </div>
          )}
        </div>
      </div>
    ...
    <div className="card p-8 bg-gradient-to-r from-blue-50 via-white to-purple-50">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-8">
        <div className="text-center group">
          ...
          <div className="display-medium text-slate-800 font-bold mb-1">{makeupArtist.experienceYears || 0}</div>
        ...
        <div className="text-center group">
          ...
          <div className="display-medium text-slate-800 font-bold mb-1">{makeupArtist.totalReviews || 0}</div>
        ...
        <div className="text-center group">
          ...
          <div className="display-medium text-slate-800 font-bold mb-1">{makeupArtist.rating || 0}</div>
        ...
        <div className="text-center group">
          ...
          <div className="body-small text-slate-500 font-medium">Makeup Sessions</div>
        </div>
      </div>
    </div>
  </div>
      ...
      <div className="text-center py-16">
        <div className="card p-12 bg-gradient-to-br from-slate-50 to-slate-100 max-w-md mx-auto">
          ...
          <p className="body-medium text-slate-500 mb-6">
            We create personalized makeup packages tailored to your event and style preferences.
          </p>
  ...
  return (
    <div className="space-y-8">
      <div className="text-center mb-10">
        <h3 className="headline-large mb-3">Makeup Packages</h3>
        <p className="body-large text-slate-600">Choose the perfect package for your special occasion</p>
      </div>
              ...
              <div className="text-center mb-8">
                ...
                <div className="mb-4">
                  ...
                  <div className="flex items-center justify-center gap-2 mt-2">
                    ...
                    <span className="body-medium text-slate-600">{pkg.durationHours} hours session</span>
                  </div>
                </div>
              </div>
...
const MakeupArtistDetail = () => {
  const { id } = useParams()
  const [makeupArtist, setMakeupArtist] = useState<any>(null)
  ...
  useEffect(() => {
    ...
    makeupArtistService
      ...
      .catch((err) => {
        setError("Failed to load makeup artist details")
        setLoading(false)
      })
  }, [id])
      ...
      <div className="container py-16">
        <div className="flex items-center justify-center min-h-[400px]">
          <div className="text-center space-y-4">
            ...
            <p className="body-large text-slate-600">Loading makeup artist details...</p>
          </div>
        </div>
      </div>
    )
  ...
  if (!makeupArtist) {
    return (
      <div className="container py-16">
        <div className="text-center">
          <div className="card p-12 max-w-md mx-auto">
            <p className="headline-medium text-slate-600 mb-4">Makeup Artist Not Found</p>
            <p className="body-medium text-slate-500 mb-6">
              The makeup artist you're looking for doesn't exist or has been removed.
            </p>
            <Link to="/client-dashboard" className="btn-primary">
              Back to Dashboard
            </Link>
      ...
      <div className="container py-8 section-spacing">
        ...
        <div className="grid lg:grid-cols-5 gap-8 mb-16">
              ...
              <div className="mt-6 space-y-4">
                <div>
                  <div className="flex items-center gap-3 mb-2">
                    <h1 className="headline-large text-slate-900">{makeupArtist.businessName}</h1>
                    {makeupArtist.user?.userType === "makeupArtist" && (
          ...
          <div className="lg:col-span-3">
            <QuickInfoSection makeupArtist={makeupArtist} />
          </div>
        </div>
        ...
        <div className="grid lg:grid-cols-3 gap-12">
          ...
          <div className="lg:col-span-1">
            <div className="sticky top-24">
              <ReviewsSection reviews={makeupArtist.reviews || []} />
            </div>
          </div>
        </div>
